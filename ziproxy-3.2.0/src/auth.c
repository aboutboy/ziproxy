...
int auth_md5_hash (const char *user, const char *pass)
{
	...
	sprintf(string ,"%s%s", user , MD5KEY);
	...
	char md5pass[32];
	memset( md5pass , 0 , strlen(md5pass) );
	
	int i=0;
	for(i=0; i < 16 ; ++i ) {
	  sprintf(md5pass, "%s%02x", md5pass, digest[i]);
	}

	if( strncmp ( md5pass ,pass,8) ) {
		debug_log_printf ("Failed authentication for %s: %s\n", string, md5pass);
		return 0;
	}

	access_log_define_username(user);
	return 1; /* success */
}
//end add
...
int auth_basic_check (const char *userpass)
{
	...
	*between = '\0';
	...
	case AUTH_PASSWD_FILE:
		...
		break;
	case AUTH_MD5_HASH:
		retcode = auth_md5_hash( user ,pass);
		break;
#ifdef SASL
	case AUTH_SASL:
