 ...
 *
 * ---------------------------------------------------------------------
 * Copyright (c)2007-2010 Daniel Mealha Cabrita
 *
...
void qp_end (t_qp_configfile *conf_handler)
{
	...
	if (conf_handler->filedata)
		free (conf_handler->filedata);
...
{
	void *rcP;
	rcP = get_single_val (conf_handler, conf_key, QP_PARM_STATUS_VALID);
	if (rcP != NULL)
	{	
		free(rcP);
		return (1);
