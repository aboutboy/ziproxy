...
int gzip_stream_stream (FILE *source, FILE *dest, int level, ZP_DATASIZE_TYPE *inlen, ZP_DATASIZE_TYPE *outlen,
                       int de_chunk, ZP_DATASIZE_TYPE content_length)
{
	int ret, flush;
	unsigned have;
	z_stream strm;
	...
	*outlen += fwrite (&gzip_header, 1, 10, dest); // gzip header
	...
	do {
		...
		*inlen += strm.avail_in;
		...
		if( content_length > 0 && *inlen == content_length) {  //got all content, so  let's go
			debug_log_puts("I got everything now,let's go...");
			flush = Z_FINISH ;
		}
		else {
			if (ferror(source)) {
